/**
 * Narrowing
 * Narrowing은 Union 타입에서 더욱 구체적인 타입으로 논리적으로 유추 할 수 있게 되는걸 의미한다.
 */

// let numberOrString: number | string = 'CoCo';
// numberOrString;

// const decimal = 98.3711;
...
// (1) Assignment Narrowing
// 특정 값을 할당해서 내로잉
let numbOrString: number | string = 'TypeScript';

numbOrString.toString();

// (2) typeof narrowing
numbOrString = Math.random() > 0.5 ? 1123 : 'TypeScript';
...
// (3) Truthiness Narrowing
let nullOrString: null | string[] = Math.random() > 0.5 ? null : ['TS', 'JS'];
...
// (4) Equality Narrowing
let numbOrString2: number | string = Math.random() > 0.5 ? 1123 : 'TS';
let stringOrBool2: string | boolean = Math.random() > 0.5 ? 'JS' : true;
...
let numberOrStringOrNull: number | string | null = Math.random() > 0.5 ? 1123 : Math.random() > 0.5 ? 'TS' : null;
...
// (6) instanceof narrowing
let dateOrString: Date | string = Math.random() > 0.5 ? new Date() : 'TS';
